        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        if final_act is None:
            ...
            self.output_linear = LinearActivation(
                # self.d_model*gate if gate is not None else self.layer.d_output, # for fftconv
                self.d_model * gate if gate is not None else self.d_model,  # for slconv
                int(self.d_model / downsample_factor),
                transposed=False,
